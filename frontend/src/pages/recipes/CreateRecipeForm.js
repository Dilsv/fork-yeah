          ...
          <Col md={6} className="justify-content-center mx-auto">
              ...
              <Form onSubmit={handleSubmit}>
                  ...
                  <Row className="justify-content-center ">
                      ...
                      <Form.Select onChange={(e) => handleAddIngredient(e.target.value)} className="mb-3 text-capitalize">
                          <option value="">Add Ingredient</option>
                          {ingredients.map((ingredient) => (
                              <option className="text-capitalize" key={ingredient.id} value={ingredient.id}>{ingredient.name}</option>
                          ))}
                      </Form.Select>
                      {errors?.ingredients && Array.isArray(errors.ingredients) && errors.ingredients.map((message, idx) => (
                          <Alert variant="warning" key={idx}>
                              {message}
                          </Alert>
                      ))}
